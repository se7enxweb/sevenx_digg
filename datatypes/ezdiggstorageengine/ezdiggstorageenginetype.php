...
class eZDiggStorageEngineType extends eZDataType
{
    ...
    {
        parent::__construct( self::DATA_TYPE_STRING, ezpI18n::tr( 'kernel/classes/datatypes', "Digg SE", 'Datatype name' ),
    ...
    */
    function initializeObjectAttribute( $contentObjectAttribute, $currentVersion, $originalContentObjectAttribute )
    {
        ...
        {
	    /* 7x implements creation of dual digg support for the undigg votes as well  before 1.0stable */
	    $diggStorageEngine = 'enabled';

            if ( $diggStorageEngine == 'enabled' )
            {
                // Add digg option to attribute

                $option = $contentObjectAttribute->content( );

                // $postvarname = "ContentObjectAttribute" . "_data_option_remove_" . $contentObjectAttribute->attribute( "id" );
                // $http->hasPostVariable( $postvarname ) )
                
                $idArray = array( '1' ); // $http->postVariable( $postvarname );
                $beforeID = array_shift( $idArray );
    ...
    {
        $multipriceTableAlias = "ci";

        if ( isset( $params['table_alias_suffix'] ) )
            $multipriceTableAlias .= $params['table_alias_suffix'];

        $sql = array( 'select' => "$multipriceTableAlias.contentobject_attribute_id, $multipriceTableAlias.contentobject_id, count( $multipriceTableAlias.contentobject_attribute_id )",
		      'from' => '',
		      'group_by' => "Group By $multipriceTableAlias.contentobject_id, $multipriceTableAlias.contentobject_attribute_id, ezcontentobject.contentclass_id, ezcontentobject.current_version, ezcontentobject_tree.contentobject_is_published,  ezcontentobject_tree.contentobject_version, ezcontentobject_tree.depth, ezcontentobject_tree.is_hidden, ezcontentobject_tree.main_node_id, ezcontentobject_tree.modified_subnode, ezcontentobject_tree.node_id,  ezcontentobject_name.name, ezcontentobject_name.real_translation",
                      'where' => "$multipriceTableAlias.contentobject_id = ezcontentobject_tree.contentobject_id",
                      'sorting_field' => "" );

        $sql['from'] =  "ezinfocollection_attribute $multipriceTableAlias";

        $and = '';
        if ( isset( $params['contentobject_attr_id'] ) )
        {
            $sql['where'] = "
                     $multipriceTableAlias.contentobject_attribute_id = {$params['contentobject_attr_id']} AND $multipriceTableAlias.data_int = 1 
			OR $multipriceTableAlias.contentobject_attribute_id = {$params['contentobject_attr_id']} AND $multipriceTableAlias.data_int = 0";
            $and = ' AND';
        }

        $sql['sorting_field'] = "count( $multipriceTableAlias.contentobject_id )";
